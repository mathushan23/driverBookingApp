...
export function RiderBookingWaitingPage({ user }) {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#020b1a] px-4 py-6 text-white">
      <BackgroundFX />
      <section className="relative z-10 mx-auto grid min-h-[calc(100vh-3rem)] max-w-7xl place-items-center">
        <motion.div
        ...
        >
          <div className="grid lg:grid-cols-[0.54fr_1fr]">
            <aside className="border-b border-blue-200/15 bg-slate-950/28 p-6 sm:p-8 lg:border-b-0 lg:border-r">
              <p className="text-xs font-black uppercase tracking-[0.42em] text-blue-300">booking request</p>
              <h1 className="mt-5 text-4xl font-black leading-tight tracking-tight sm:text-5xl">{statusTitle}</h1>
              <p className="mt-4 text-base font-semibold leading-7 text-blue-100/72">{statusText}</p>
              ...
              <div className="mt-4 grid grid-cols-2 overflow-hidden rounded-[1.45rem] border border-white/10 bg-white/[0.06]">
                <div className="border-r border-white/10 p-4">
                  <div className="grid h-11 w-11 place-items-center rounded-2xl bg-blue-500/18 text-lg font-black text-blue-300">DR</div>
                  <p className="mt-3 text-xs font-bold text-blue-100/60">Nearby Drivers</p>
                  <p className="mt-1 text-3xl font-black text-blue-300">{booking?.nearbyDriverCount ?? displayDrivers.length}</p>
                  <p className="mt-1 text-xs font-semibold text-blue-100/55">Available</p>
                </div>
                <div className="p-4">
                  <div className="grid h-11 w-11 place-items-center rounded-2xl bg-emerald-400/14 text-lg font-black text-emerald-300">ET</div>
                  <p className="mt-3 text-xs font-bold text-blue-100/60">Estimated Wait</p>
                  <p className="mt-1 text-2xl font-black text-emerald-300">{accepted ? driverEta || 'Soon' : waitEstimate(displayDrivers.length)}</p>
              ...
              {!accepted && !canceled && (
                <button onClick={cancelRide} disabled={canceling} className="mt-6 flex w-full items-center justify-center gap-3 rounded-[1.35rem] border border-red-400/45 bg-red-500/10 px-5 py-4 text-sm font-black text-red-100 transition hover:bg-red-500/20 disabled:opacity-60">
                  <span className="grid h-11 w-11 place-items-center rounded-full bg-red-500/22 text-2xl leading-none">x</span>
            ...
            <section className="p-5 sm:p-8">
              <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  <span className="grid h-10 w-10 place-items-center rounded-full bg-blue-500/15 text-blue-300 ring-1 ring-blue-300/15">SR</span>
              ...
              <div className="mt-5 rounded-[1.35rem] border border-blue-300/15 bg-[linear-gradient(90deg,rgba(37,99,235,.16),rgba(15,23,42,.32))] p-5">
                <div className="flex items-center gap-4">
                  <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-blue-500/18 text-xl font-black text-blue-300">SH</div>
                  <div>
                    <h2 className="text-xl font-black">Your safety is our priority</h2>
...
function RouteInfoCard({ tone, label, value }) {
  ...
  return (
    <div className="flex items-center gap-4 rounded-[1.35rem] border border-white/10 bg-white/[0.05] p-4">
      <div className={`grid h-14 w-14 shrink-0 place-items-center rounded-2xl ${tones[tone] || tones.blue}`}>PIN</div>
      <div className="min-w-0">
        <p className="text-sm font-semibold text-blue-100/65">{label}</p>
...
function DriverSearchMap({ accepted, canceled, drivers, booking }) {
  ...
  return (
    <div className="relative min-h-[520px] overflow-hidden rounded-[1.5rem] border border-blue-300/20 bg-[#071832] shadow-2xl shadow-blue-950/40">
      {isLoaded && apiKey ? (
        <GoogleMap
        ...
        <div className="grid h-[520px] place-items-center px-6 text-center text-sm font-bold text-blue-100">
