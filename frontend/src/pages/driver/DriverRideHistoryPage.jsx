...
export function DriverRideHistoryPage({ user }) {
    ...
    <main className="relative min-h-screen overflow-hidden bg-slate-950 px-4 py-6 text-white">
      ...
      <section className="relative z-10 mx-auto max-w-6xl">
        <header className="mb-6 flex flex-col gap-4 rounded-[2rem] border border-white/15 bg-white/10 p-5 shadow-2xl shadow-blue-950/40 backdrop-blur-2xl sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link to="/driver/dashboard" className="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-black text-blue-100 hover:bg-white/15">Back</Link>
        </header>
        ...
        <div className="grid gap-4">
          {rides.map((ride, index) => (
            <motion.article key={ride.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={`rounded-[1.5rem] border p-5 ${ride.status === 'ACCEPTED' ? 'border-emerald-300/35 bg-emerald-500/15' : 'border-white/10 bg-white/10'}`}>
